...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│   Language Model│  (gpt-5.1, claude-3, qwen2.5-72b-instruct, etc.)
└─────────────────┘
```
...
<CardGroup cols={3}>
  <Card title="OpenAI Provider" icon="bolt">
    Full OpenAI API support including GPT-5.1, Codex, embeddings, and more
  </Card>
  <Card title="Anthropic Provider" icon="sparkles">
    Full Anthropic API support including Claude 4.5, Claude 4, and more
  </Card>
  <Card title="Qwen Provider" icon="zap">
...
Language models are instances created from providers. They represent a specific model (e.g., `gpt-5.1`, `gpt-4o`) and can be used with core functions.
